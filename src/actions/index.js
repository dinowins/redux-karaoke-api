...
export function fetchSongId(title){
  return function (dispatch){
    ...
    return fetch('http://api.musixmatch.com/ws/1.1/track.search?&q_track=' + title + '&page_size=1&s_track_rating=desc&apikey=294b0c5b9d65363bdf37076d24caba03').then(
      ...
      if (json.message.body.track_list.length > 0) {
        ...
        console.log('We could\'t locate a song under that ID!');
      }
    });
  };
}
...
export function fetchLyrics(title, artist, musicMatchId, localSongId, dispatch) {
  return fetch('http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=' + musicMatchId + '&apikey=294b0c5b9d65363bdf37076d24caba03').then(
    response => response.json(),
